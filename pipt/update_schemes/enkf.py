...
class enkfMixIn(Ensemble):
    ...
    def calc_analysis(self):
        ...
        if not hasattr(self, 'prior_data_misfit'):
            ...
            list_datatypes, list_active_dataypes = at.get_list_data_types(self.obs_data, assim_index)
            if not hasattr(self,'cov_data'):
                self.full_cov_data = at.gen_covdata(self.datavar, assim_index, list_datatypes)
            else:
                 self.full_cov_data = self.cov_data
            obs_data_vector, pred_data = at.aug_obs_pred_data(self.obs_data, self.pred_data, assim_index,list_datatypes)
            # Generate realizations of the observed data
            init_en = Cholesky()  # Initialize GeoStat class for generating realizations
            self.full_real_obs_data = init_en.gen_real(obs_data_vector, self.full_cov_data, self.ne)
