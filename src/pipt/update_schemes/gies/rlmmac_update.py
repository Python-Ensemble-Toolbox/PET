...
class rlmmac_update():
    """
     Regularized Levenburg-Marquardt algorithm for Minimum Average Cost (RLM-MAC) problem, following the paper:
     Luo, Xiaodong, et al. "Iterative ensemble smoother as an approximate solution to a regularized
    ...
    def update(self):
        ...
        if 'localization' in self.keys_da:
            if 'emp_cov' in self.keys_da and self.keys_da['emp_cov'] == 'yes':
                ...
                # assume S_d = U S V^T; then Kalman gain K = S_m S_d^T (S_d S_d^T + gamma E E^T)^{-1} (d^o - g(m))
                # For this particular part A
                # = S_d^T (S_d S_d^T + gamma E E^T)^{-1}
                # = ...
                # = V (I + gamma (S^-1 U^T E) (S^-1 U^T E)^T)^{-1} (S^-1 U^T)
                # let S^-1 U^T E = Z L Z^T
                # = V Z (I + gamma L)^{-1} Z^T S^-1 U^T
                alpha = self.lam * np.sum(Lam**2) / len(Lam)
                X = v_d.T @ z @ np.diag(1 / (alpha * Lam + np.eye(len(Lam)))) @ z.T @ np.diag(s_d[:] ** (-1)) @ u_d.T
                #X = np.dot(np.dot(v_d.T, z), solve((self.lam + 1) * np.diag(Lam) + np.eye(len(Lam)),
                #                                   np.dot(u_d[:, :], np.dot(np.diag(s_d[:] ** (-1)).T, z)).T))

            else:
                alpha = self.lam * np.sum(s_d**2) / len(s_d)
                X = v_d.T @ np.diag(s_d / (s_d ** 2 + alpha)) @ u_d.T
                #X = np.dot(np.dot(v_d.T, np.diag(s_d)), solve(((self.lam + 1) * np.eye(len(s_d)) + np.diag(s_d ** 2)), u_d.T))
