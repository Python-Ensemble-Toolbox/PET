...
class Ensemble:
    ...
    def calc_modeling_error_sep(self):
        #todo: this has not been fully implemented for telescopic version of error
        Sample_size=int(self.ml_ne[self.tot_level - 1]/2)
        #Sample_size=5
        tmp = np.random.permutation(self.ml_ne[0])
        list_states = list(self.state.keys())
        dummy=at.aug_state(self.ML_state[0], list_states)
        dummy=dummy[:,tmp[0:Sample_size]]
        #self.state=self.update_state(dummy,self.ML_state[0],list_states)
        '''
        for _, key in enumerate(list_states):
            # Take a random sample from coarsest level
            self.state[key] = deepcopy(self.ML_state[0][key][:, tmp[0:Sample_size]])
        '''
        Temp_predict=[]
        for level in range(self.tot_level):
            for k in range(len(self.obs_data)):
                self.obs_data[k][self.keys_fwd['datatype']] = np.zeros((self.data_size[level], 1)).flatten()
            self.sim.level = level
            self.sim.flow.level = level
            self.ne = Sample_size
            self.state = self.update_state(dummy, self.ML_state[0], list_states)
            self.calc_forecast(0)
            Temp_predict.append(deepcopy(self.pred_data))

        assim_step = 0
        assim_index = [self.keys_da['obsname'], self.keys_da['assimindex'][assim_step]]
        list_datatypes, _ = at.get_list_data_types(self.obs_data, assim_index)
        tot_pred = []
        for level in range(self.tot_level):
            obs_data_vector, pred = at.aug_obs_pred_data(self.obs_data, Temp_predict[level], assim_index,
                                                         list_datatypes)  # get some data
            # pred = self.Dns_mat[level] * pred
            tot_pred.append(pred)

        # Ref=[None]*len(assim_index)
        # for ind in range(len(assim_index)):
        #    for key in self.ML_predict[self.tot_level-1][ind]:
        #        Ref[ind]=np.mean(self.ML_predict[self.tot_level-1][ind][key],axis=1)
        if self.ML_error_corr == 'bias_corr':
            self.Bias = []
            # Ref = np.mean(self.Dns_Mat[self.tot_level-1]*tot_pred[self.tot_level - 1], axis=1)
            Ref = np.mean(tot_pred[self.tot_level - 1], axis=1)
            Ref = np.reshape(Ref, (Ref.shape[0], 1))
            # Ref=np.mean(tot_pred[self.tot_level-1],axis=1)
            for level in range(self.tot_level - 1):
                L_mean = np.mean(tot_pred[level], axis=1)
                L_mean = np.reshape(L_mean, (L_mean.shape[0], 1))
                # self.Bias.append(self.B[level] * Ref - L_mean)
                self.Bias.append(Ref - L_mean)

        elif self.ML_error_corr == 'stochastic':
            Ref = tot_pred[self.tot_level - 1]
            for level in range(self.tot_level - 1):
                TmpR = self.B[level] * Ref
                TmpL = (tot_pred[level][:, 0:self.ml_ne[self.tot_level - 1]])
                Stat = TmpR - TmpL
                Mean = np.mean(Stat, axis=1).reshape((Stat.shape[0], 1))
                Stat = Stat - np.matmul(Mean, np.ones((1, Stat.shape[1])))
                Half_Cov = (1 / np.sqrt(self.ml_ne[level] - 1)) * Stat
                self.error_mean.append(Mean)
                self.error_half_cov.append(Half_Cov)

        elif self.ML_error_corr == 'telescopic':
            self.Proj_E = []
            self.Exp_E = []
            self.Proj_State = []
            self.Exp_state = []
            list_states = list(self.state.keys())
            for level in range(self.tot_level - 2, -1, -1):
                Ref = self.Dns_mat[level + 1] * tot_pred[level + 1]
                TmpR = self.B[level] * Ref
                TmpL = tot_pred[level][:, 0:self.ml_ne[level + 1]]
                Stat = TmpR - TmpL
                Mean_stat = np.mean(Stat, axis=1).reshape((Stat.shape[0], 1))
                Proj_Stat = Stat - np.matmul(Mean_stat, np.ones((1, Stat.shape[1])))
                self.Proj_E.append(Proj_Stat)
                self.Exp_E.append(Mean_stat)
                aug_state = at.aug_state(self.ML_state[level+1], list_states)
                Mean_state = np.mean(aug_state, axis=1).reshape((aug_state.shape[0], 1))
                Proj_aug_state = aug_state - np.matmul(Mean_state, np.ones((1, aug_state.shape[1])))
                self.Proj_State.append(Proj_aug_state)
                self.Exp_state.append(Mean_state)

        elif self.ML_error_corr == 'deterministic':
            self.Proj_E = []
            self.Exp_E = []
            self.Proj_State = []
            self.Exp_state = []
            ...
            for level in range(self.tot_level - 2, -1, -1):
                ...
                self.Proj_E.append(Proj_Stat)
                self.Exp_E.append(Mean_stat)
                aug_state = at.aug_state(self.ML_state[self.tot_level-1], list_states)
                Mean_state = np.mean(aug_state, axis=1).reshape((aug_state.shape[0], 1))
                Proj_aug_state = aug_state - np.matmul(Mean_state, np.ones((1, aug_state.shape[1])))
                self.Proj_State.append(Proj_aug_state)
                self.Exp_state.append(Mean_state)
    ...
    def calc_modeling_error_ens(self):
        ...
        if self.ML_error_corr =='bias_corr':
            ...
            for elem in self.assim_index[1]:
                for dat in self.list_datatypes:
            ...
            self.error_mean=[]
            self.error_half_cov=[]
            Ref = self.Dns_mat[self.tot_level-1]*tot_pred[self.tot_level - 1]
            for level in range(self.tot_level - 1):
                #TmpR = self.B[level] * Ref
                TmpR = Ref
                TmpL = (tot_pred[level][:, 0:self.ml_ne[self.tot_level - 1]])
        ...
        elif self.ML_error_corr == 'teles_bias':
            self.Bias = []
            Exp_E = []
            list_states = list(self.state.keys())
            for level in range(self.tot_level - 2, -1, -1):
                Ref = self.Dns_mat[level + 1] * tot_pred[level + 1]
                TmpL = self.Dns_mat[level] * tot_pred[level][:, 0:self.ml_ne[level + 1]]
                Stat = Ref - TmpL
                Mean_stat = np.mean(Stat, axis=1).reshape((Stat.shape[0], 1))
                Exp_E.append(Mean_stat)
            for level in range(self.tot_level-1):
                Dummy = sum(Exp_E[0:self.tot_level-1 - level])
                Dummy = self.B[level] * Dummy
                self.Bias.append(Dummy)
            self.Bias.append(np.zeros((Exp_E[0].shape[0],1)))
    ...
    def address_ML_error(self):
        ...
        # As the converged levels are not simulated again, they should not be corrected either
        # for l in range(self.tot_level-1):
        #     if iteration>0:
        #         if self.level_conv[l]:
        #             if self.ML_error_corr == 'bias_corr':
        #                 self.Bias[l] -= self.Bias[l]
        #             if self.ML_error_corr == 'stochastic':
        #                 self.error_mean[l] -= self.error_mean[l]
        #                 self.error_half_cov[l] -= self.error_half_cov[l]

        if self.ML_error_corr in ['bias_corr', 'teles_bias']:
            #Ref=np.mean(tot_pred[self.tot_level-1],axis=1)
            for l in range(self.tot_level - 1):
                for ind in range(len(self.assim_index)):
                    for key in self.ML_predict[l][ind]:
                        Tmp=np.dot(self.Bias[l],np.ones((1,self.ml_ne[l])))
                        #L_mean=np.mean(self.ML_predict[l][ind][key],axis=1)
                        self.ML_predict[l][ind][key]+=Tmp[ind*self.data_size[l]:(ind+1)*self.data_size[l],:]
                        '''
                        self.ML_predict[l][ind][key][:,0:self.ml_ne[self.tot_level-1]]=(self.B[l]* \
                                    self.Dns_mat[self.tot_level-1] * \
                                    tot_pred[self.tot_level-1])[ind*self.data_size[l]:(ind+1)*self.data_size[l],:]
                        '''

        elif self.ML_error_corr == 'stochastic':
            for level in range(self.tot_level - 1):
                for ind in range(len(self.assim_index)):
                    for key in self.ML_predict[level][ind]:
                        #L_mean=np.mean(self.ML_predict[l][ind][key],axis=1)

                        Error_samp = np.dot(self.error_mean[level],np.ones((1,self.ml_ne[level])))+np.dot(
                            self.error_half_cov[level],
                            np.random.randn(self.error_half_cov[level].shape[1], self.ml_ne[level]))

                        #Error_samp = np.dot(self.error_mean[level], np.ones((1, self.ml_ne[level])))
                        self.ML_predict[level][ind][key]+=Error_samp[ind*self.data_size[level] \
                                                                 :(ind+1)*self.data_size[level],:]
                        self.ML_predict[level][ind][key][:,0:self.ml_ne[self.tot_level-1]]=(self.B[level] * \
                                tot_pred[self.tot_level-1])[ind*self.data_size[level]:(ind+1)*self.data_size[level],:]


        elif self.ML_error_corr == 'telescopic':

            list_states = list(self.state.keys())
            aug_state = [at.aug_state(self.ML_state[elem], list_states) for elem in range(self.tot_level)]
            '''
            for err in range(self.tot_level-1):
                Error_deriv = np.matmul(self.Proj_E[err], self.calc_pinv(self.Proj_State[err]))
                for level in range(0,self.tot_level -1 -err,1):
                    for ind in range(len(assim_index)):
                        for key in self.ML_predict[level][ind]:
                            Correc_term=np.matmul(self.Exp_E[err],np.ones((1,aug_state[level].shape[1]))) + \
                                np.matmul(Error_deriv,aug_state[level]-np.matmul(self.Exp_state[err],
                                                                    np.ones((1,aug_state[level].shape[1]))))
                            self.ML_predict[level][ind][key] +=Correc_term[ind*self.data_size[level] \
                                                                 :(ind+1)*self.data_size[level],:]
            for level in range(0, self.tot_level - 1 - err, 1):
                for ind in range(len(assim_index)):
                    for key in self.ML_predict[level][ind]:
                        self.ML_predict[level][ind][key][:, 0:self.ml_ne[self.tot_level - 1]]= ( self.B[level] * \
                                tot_pred[self.tot_level-1])[ind*self.data_size[level]:(ind+1)*self.data_size[level],:]
            '''
            ml_ne=deepcopy(self.ml_ne)
            ml_ne.reverse()
            Sizes = [0]
            Sizes.extend(ml_ne)
            ml_ne.reverse()
            for err in range(self.tot_level - 1):
                for level in range(self.tot_level - err -1):
                    for ind in range(len(self.assim_index)):
                        for key in self.ML_predict[level][ind]:
                            self.ML_predict[level][ind][key][:, Sizes[err]:Sizes[err+1]] = (self.B[level] * \
                                self.Dns_mat[self.tot_level-1 -err] *np.concatenate(\
                                        [self.ML_predict[self.tot_level-1-err][ii][key][ \
                                        :,Sizes[err]:Sizes[err+1]] for ii in range(len(self.assim_index))] , axis=0))[ \
                                        ind*self.data_size[level]:(ind+1)*self.data_size[level],:]
                Error_deriv = np.matmul(self.Proj_E[err], self.calc_pinv(self.Proj_State[err]))
                for level in range(self.tot_level-err-1):
                    Correc_term = self.B[level]* self.Dns_mat[self.tot_level-err -2]* \
                                  (np.matmul(self.Exp_E[err], np.ones((1,ml_ne[level]-Sizes[err+1]))) + \
                                    np.matmul(Error_deriv,aug_state[level][:,Sizes[err+1]:] - \
                                    np.matmul(self.Exp_state[err],np.ones((1,ml_ne[level]-Sizes[err+1])))))
                    for ind in range(len(self.assim_index)):
                        for key in self.ML_predict[level][ind]:
                            self.ML_predict[level][ind][key][:,Sizes[err+1]:]+= Correc_term[ind*self.data_size[level] \
                                                                            :(ind+1)*self.data_size[level],:]
            ...
            aug_state = [at.aug_state(self.ML_state[elem], list_states) for elem in range(self.tot_level)]
            for level in range(self.tot_level -1):
                Error_deriv = np.matmul(self.Proj_E[level], self.calc_pinv(self.Proj_State[level]))
                for ind in range(len(self.assim_index)):
                    for key in self.ML_predict[level][ind]:
                        Correc_term=np.matmul(self.Exp_E[level],np.ones((1,aug_state[level].shape[1]))) + \
                            np.matmul(Error_deriv,aug_state[level]-np.matmul(self.Exp_state[level],
                                                                np.ones((1,aug_state[level].shape[1]))))
                        self.ML_predict[level][ind][key] +=Correc_term[ind*self.data_size[level] \
                                                             :(ind+1)*self.data_size[level],:]
                        self.ML_predict[level][ind][key][:, 0:self.ml_ne[self.tot_level - 1]] = (self.B[level] * \
